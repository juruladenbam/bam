...
export function SubmissionsPage() {
    ...
    // We only fetch submissions if authenticated
    const { isAuthenticated, linkedPerson, loginEnabled, isNibLinked } = usePortalMode()

    const { data: submissions, isLoading: submissionsLoading } = useMySubmissions()

    // Determine access state:
    // - If login enabled: requires auth + linked person
    // - If login disabled: requires NIB linked (stored in session)
    const hasAccess = loginEnabled
        ? (isAuthenticated && !!linkedPerson)
        : isNibLinked;

    // What kind of access is needed?
    // - If login enabled but not authenticated: need login
    // - If login disabled but no NIB: need to link NIB
    const needsNibLink = !loginEnabled && !isNibLinked;
    const needsLogin = loginEnabled && !isAuthenticated;

    // Check if we came from Silsilah (has preselectedPerson)
    const fromSilsilah = !!location.state?.preselectedPerson

    useEffect(() => {
        if (fromSilsilah && hasAccess) {
            setShowForm(true)
        }
    }, [fromSilsilah, hasAccess])

    const handleCancel = () => {
        if (fromSilsilah) {
            // Navigate back to the specific branch and person
            const person = location.state.preselectedPerson
            const branchId = location.state.fromBranchId || person.branch_id
    ...
    // If not authenticated, we don't load submissions, so just ready/not loading
    const isLoading = isAuthenticated ? submissionsLoading : false;
    ...
    return (
        <MobileLayout>
            <div className="relative flex-1">
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl border border-[#e6dbdc] p-8 max-w-sm w-full text-center animate-in fade-in zoom-in duration-300">
                            <div className="size-16 rounded-full bg-[#ec1325]/10 flex items-center justify-center mx-auto mb-6 text-[#ec1325]">
                                <span className="material-symbols-outlined text-4xl">
                                    {needsLogin ? 'lock' : 'link_off'}
                                </span>
                            </div>
                            <h2 className="text-xl font-bold text-[#181112] mb-3">
                                {needsLogin
                                    ? 'Login Diperlukan'
                                    : needsNibLink
                                        ? 'Tautkan NIB Anda'
                            ...
                            <p className="text-[#896165] text-sm mb-8 leading-relaxed">
                                {needsLogin
                                    ? 'Fitur laporan data memerlukan akun pengguna untuk melacak status laporan Anda.'
                                    : needsNibLink
                                        ? 'Untuk melapor data, silakan tautkan Nomor Induk BAM (NIB) Anda terlebih dahulu.'
                            ...
                            {needsLogin ? (
                                ...
                                <Link
                                    ...
                                    className="block w-full bg-[#ec1325] text-white font-bold py-3 rounded-xl hover:bg-red-600 transition-colors shadow-lg shadow-red-200"
                                >
                                    {needsNibLink ? 'Tautkan NIB' : 'Tautkan Akun'}
                                </Link>
                            )}
                ...
                <div className={`flex-1 max-w-4xl mx-auto w-full px-4 md:px-10 py-8 ${!hasAccess ? 'h-[80vh] overflow-hidden' : ''}`}>
                    {/* Header */}
                    ...
                    {showForm && hasAccess ? (
                        ...
                        <>
                            {/* Stats */}
                            {submissions && submissions.length > 0 && hasAccess && (
